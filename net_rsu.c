	...
	while(1)
		{
			...
			else
			{
				...
				{
					stuRsuData.RSUStatus=buf[9];
					stuRsuData.PSAMCount=buf[10];
					printf("B0_psam num is %x\n",stuRsuData.PSAMCount);
					t=buf[10];
					for(i=0;i<t;i++)
					{
						stuRsuData.PSAMInfoN[i].PSAM_channel=buf[11+9*i];	
						stuRsuData.PSAMInfoN[i].PSAM_version=buf[12+9*i];
						stuRsuData.PSAMInfoN[i].PSAM_auth=buf[13+9*i];
						printf("psam_ID is:");
						for(j=0;j<6;j++)
						{
							stuRsuData.PSAMInfoN[i].Psam_ID[j]=buf[14+9*i+j];
							printf(" %x-",stuRsuData.PSAMInfoN[i].Psam_ID[j]);
						}
						printf("\n");
					}
					stuRsuData.RSUAlgId=buf[11+9*t];
					stuRsuData.RSUManuID=buf[12+9*t];
					printf("RSUManuID is %x\n",stuRsuData.RSUManuID);
					for(j=0;j<3;j++)
					{stuRsuData.RSUID[j]=buf[13+9*t+j];}
					for(j=0;j<2;j++)
					{stuRsuData.RSUVersion[j]=buf[16+9*t+j];}
					stuRsuData.Workstatus=buf[18+9*t];
					for(j=0;j<3;j++)
					{stuRsuData.FlagID[j]=buf[19+9*t+j];}
					for(j=0;j<4;j++)
					{stuRsuData.Reserved[j]=buf[22+9*t+j];}
				}
