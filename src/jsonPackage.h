...
bool jsonStrVMCtlParamWriterXY(int messagetype,char *pstrDevInfo, char *json, int *len);//12 控制器参数->JSON字符串(新粤)
...
bool jsonStrRsuWriterXY(int messagetype,char *json, int *len);//14 RSU天线参数
...
bool jsonStrSwitchStatusWriterXY(int messagetype, char *json, int *len);	//19回路电压电流开关状态
